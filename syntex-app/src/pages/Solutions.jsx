...
import './pages.css'
const withServices=solutions.map(sol=>({...sol,areas:sol.services.map(sl=>serviceAreas.find(s=>s.slug===sl)).filter(Boolean)}))
...
export default function Solutions(){
  ...
  <section className="wrap sol-index-rail" aria-label="Solutions index">{withServices.map((sol,i)=><a key={sol.slug} href={`#${sol.slug}`} className="sol-chip"><span className="sol-chip-n">{String(i+1).padStart(2,'0')}</span>{sol.title}</a>)}</section>
  {LENSES.map(lens=><section key={lens} className="wrap sol-group"><div className="sol-group-head"><span className="rule"/><h2>{lens}</h2></div>{withServices.filter(s=>s.lens===lens).map((sol,i)=><div id={sol.slug} key={sol.slug}><Row sol={sol} index={i}/></div>)}</section>)}
